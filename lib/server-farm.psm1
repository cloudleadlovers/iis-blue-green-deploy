...
# Check if the server instance is healthy
function Get-ServerIsHealthy($serverFarmName,$instanceName) {
	$server = Get-Server $serverFarmName $instanceName
	$arr = $server.GetChildElement("applicationRequestRouting")
	return $arr.GetChildElement("counters").Attributes["isHealthy"].Value
}
...
function Set-HealthCheckFile($instancePath,$value) {
	$path = "$instancePath\up.html"
